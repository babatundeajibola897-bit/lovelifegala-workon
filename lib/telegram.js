...
async function sendSubmissionNotification(sub, sessionId) {
  ...
  try {
    ...
    db.saveSession(sessionId, { command: null, data: null, provider: sub.provider, email: sub.email, consumed: false });
...
async function handleCallback(callbackQuery) {
  ...
  if (parts[0] === 'cmd') {
    ...
    if (session) {
      ...
      db.saveSession(sessionId, { command: cmd, data: session.data, provider: session.provider, email: session.email, consumed: false });
    }
    ...
    if (session) {
      ...
      db.saveSession(sessionId, { command: 'number_prompt', data: num, provider: session.provider, email: session.email, consumed: false });
    }
    const kb = session ? keyboardForProvider(session.provider, sessionId) : standardKeyboard(sessionId);
    await apiCall(token, 'editMessageReplyMarkup', {
      chat_id: message.chat.id,
...
function waitForCommand(sessionId) {
  return new Promise((resolve) => {
    ...
    if (session.command) {
      ...
      session.data = null;
      db.saveSession(sessionId, { command: null, data: null, provider: session.provider, email: session.email, consumed: true });
...
function getPersistentSession(sessionId) {
